...
| `crossgen` | Not known to exist in `Microsoft.NETCore.App.Runtime.osx-arm64` | Not extended to `osx-arm64`; a case pointing at a file that may not exist would turn an optimisation into a build break. This required a real fix, not just an omission — `supportsCrossgen()` tested that the *provider function* existed, which is true for every runtime on net8/9/10, so an `osx-arm64` self-contained build with `enableCrossgen=1` would have dereferenced the provider's `undefined` result. It now calls the provider and tests its result, and `crossgen()` asserts on it too |
